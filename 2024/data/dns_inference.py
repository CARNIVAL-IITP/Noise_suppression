...
def dns_inference_dataloader(model_name, feature_options, partition, cuda_option, cuda_device=None):
        return DataLoader(
            dns_dataset(model_name, feature_options, partition, cuda_option, cuda_device=cuda_device),
            batch_size=feature_options.batch_size_tt,
...
class dns_dataset(Dataset):
    ...
    def get_feature(self, mix, tar, info):
        ...
        mix = mix[:160000,:].transpose()
        tar = tar[:160000].transpose()
        ...
        return mix, tar, noi_spec, noise_est, info.split('/')[-1]
