...
class trainer:
    def __init__(self, args):
        ...
        if  self.wandb:
            wandb.init(project="iitp4", save_code=True)
    ...
    def train(self, epoch):
        ...
        for i, data in enumerate(self.train_loader):
            torch.cuda.empty_cache()
            input, label, noise, noise_est, _ = data
            ...
            for j, loss_f in enumerate(self.loss_fn):
                
                loss_s= self.args.loss_weight[j]*loss_f(out_wav, label)
                loss +=loss_s
                loss_log['tr_'+self.loss_name[j]+'_spc']=loss_s.item()
            
            losses.update(loss.item())
            loss_log['tr_loss']=losses.avg
            ...
            for a in [out_wav, est_phase, specs_t, label, input, noise, loss, loss_s, loss_log]:
                del a
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.valid_loader):
                torch.cuda.empty_cache()
                input, label, noise, noise_est, info = data
                label = label.to(self.device).float()
                input = input.to(self.device).float()
                noise = noise.to(self.device).float()
                noise_est = noise_est.to(self.device).float()
                ...
                for j, loss_f in enumerate(self.loss_fn):
                    ...
                    loss_s= self.args.loss_weight[j]*loss_f(out_wav, label)
                    loss +=loss_s
                    loss_log['val_'+self.loss_name[j]+'_spc']=loss_s.item()
            

                losses.update(loss.item())
                loss_log['val_loss']=losses.avg
                ...
                if i==0:
                    sf.write(f'{self.validation_check_path}input.wav', input[0,:8,:].cpu().detach().numpy().squeeze().T, 16000, subtype='PCM_16')
                    sf.write(f'{self.validation_check_path}output.wav', out_wav[0,:8,:].cpu().detach().numpy().squeeze().T, 16000, subtype='PCM_16')
                    sf.write(f'{self.validation_check_path}target.wav', label[0,:8,:].cpu().detach().numpy().squeeze().T, 16000, subtype='PCM_16')
                    
                if self.wandb:
                    wandb.log(loss_log)

                for a in [out_wav, est_phase, specs_t, label, input, noise, loss]:
                    del a
