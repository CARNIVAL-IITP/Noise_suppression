...
def compute_metrics(cleanFile, enhancedFile, Fs, path=None):
    alpha = 0.95
    ...
    SISDR = np.mean(sisnr(data1, data2))

    # compute the pesq
    pesq_mos = pesq(data1, data2)
    ...
    return pesq_mos, CSIG, CBAK, COVL, snr_mean, SISDR, STOI


def sisnr(s,x, remove_dc=False):
    """
    Compute SI-SNR
    Arguments:
    ...
    if remove_dc:
        x_zm = x - np.mean(x)
        s_zm = s - np.mean(s)
        t = np.zeros(x_zm.shape[0], x_zm.shape[1])

        for i in range(x_zm.shape[0]):
            t[i,:] = np.dot(x_zm[i,:], s_zm[i,:]) * s_zm[i,:] / np.norm(s_zm[i,:],2) ** 2
...
def wss(clean_speech, processed_speech, sample_rate):
    ...
    # print(clean_length, processed_length)
    # exit()
...
def llr(clean_speech, processed_speech,sample_rate):
    ...
    for frame_count in range(num_frames):
        ...
        denominator = np.dot(np.matmul(A_clean, toeplitz(R_clean)), A_clean)+1e-8
        distortion[frame_count] = np.log(numerator / denominator+1e-8)
        start = start + skiprate
    return distortion
...
def lpcoeff(speech_frame, model_order):
    ...
    for i in range(model_order):
        ...
        rcoeff[i] = (R[i + 1] - sum_term) / (E[i]+1e-8)
        a[i] = rcoeff[i]
        if i == 0:
...
def removeSilentFrames(x, y, dyrange, N, K):
    ...
    msk = 20 * np.log10(np.divide(norm(np.multiply(x[jj_list], w), axis=1), np.sqrt(N)+1e-8)+1e-8)
