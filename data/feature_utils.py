...
def get_istft(f, window_size, hop_size):
    """
    fn: the absolute path of the wav file
    sampling_rate: in Hz
    window_size: window size for fft
    ...
    return:
        stft: frame * frequency numpy array
    """
    # sig = np.exp(f*np.log(10))
    sig = librosa.core.istft(np.transpose(f), hop_length=hop_size, win_length=window_size)
    # if fs != sampling_rate:
    #     # print("WARNING!!! The sampling rate provided is different from the data")
    #     # print("Resample the audio...")
    #     sig = librosa.core.resample(sig, fs, sampling_rate)
...
def get_log_magnitude_speakerbeam(stft, epsilon=1e-7):
    feature = np.log10(np.abs(stft) + epsilon)
    # feature = librosa.util.normalize(feature, axis=1)
    return feature

def get_log_magnitude_inference(stft, epsilon=1e-7):
    feature = np.log10(np.abs(stft) + epsilon)
    max_feat = np.max(feature, axis=1)
    feature = librosa.util.normalize(feature, axis=1)
    return feature, max_feat
