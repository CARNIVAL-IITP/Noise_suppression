...
class stft:
    ...
    def compute(self, sig, samp_rate, nfft, window_time=256/16000, shift=128/16000):
        ...
        if window_time > sig.shape[-1]/samp_rate:
            ...
            sys.exit(1)
        if sig.shape[-1]%128!=0:
            sig = np.concatenate([sig, np.zeros(128 - sig.shape[-1]%128)], -1)
        ...
        return np.concatenate([np.abs(fftpack.fft(window*sig[:, i:i+window_length], nfft)[:, :nfft//2]) for i in range(0, sig.shape[-1]-(window_length-self.shift), self.shift)],0)
