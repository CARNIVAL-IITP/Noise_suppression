...
class NavieComplexLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, projection_dim=None, bidirectional=False, batch_first=False):
        ...
        self.real_lstm = nn.LSTM(self.input_dim, self.rnn_units, num_layers=1, bidirectional=bidirectional, batch_first=False).to(self.device)
        self.imag_lstm = nn.LSTM(self.input_dim, self.rnn_units, num_layers=1, bidirectional=bidirectional, batch_first=False).to(self.device)
        if bidirectional:
            bidirectional=2
        else:
            bidirectional=1
        if projection_dim is not None:
            ...
            self.r_trans = nn.Linear(self.rnn_units*bidirectional, self.projection_dim).to(self.device)
            self.i_trans = nn.Linear(self.rnn_units*bidirectional, self.projection_dim).to(self.device)
        else:
    ...
    def forward(self, inputs, state=None):
        ...
        if state != None:
            [(h_rr, c_rr), (h_ri, c_ri), (h_ir, c_ir), (h_ii, c_ii)] = state
            # print(h_rr)
            r2r_out, (h_rr, c_rr) = self.real_lstm(real, (h_rr, c_rr))    
            r2i_out, (h_ri, c_ri) = self.imag_lstm(real, (h_ri, c_ri))
            i2r_out, (h_ir, c_ir) = self.real_lstm(imag, (h_ir, c_ir))
            i2i_out, (h_ii, c_ii) = self.imag_lstm(imag, (h_ii, c_ii))
            ...
            return real_out, imag_out, [(h_rr, c_rr), (h_ri, c_ri), (h_ir, c_ir), (h_ii, c_ii)]
        else:
...
class ComplexConv2d(nn.Module):
    ...
    def forward(self,inputs):
        ...
        if inputs.dim() == 3:
            b, chf, t = inputs.shape
            inputs = inputs.reshape(b, self.in_channels, chf//self.in_channels, t)
...
class ComplexConvTranspose2d(nn.Module):
    ...
    def forward(self,inputs):
        if inputs.dim() == 3:
            mix, ch, t = inputs.shape
            inputs = inputs.reshape(16, ch, mix//16, t)
