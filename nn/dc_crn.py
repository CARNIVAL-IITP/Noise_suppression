...
from nn.conv_stft import ConvSTFT, ConviSTFT

from nn.complexnn import ComplexConv2d, ComplexConvTranspose2d, NavieComplexLSTM, complex_cat, ComplexBatchNorm
...
class DCCRN(nn.Module):
    ...
    def __init__(self,
        ...
        self.enhance1 = NavieComplexLSTM(
            input_size=4 * self.kernel_num[-1],
            hidden_size=self.rnn_units,
            bidirectional=False,
            batch_first=False,
            projection_dim=None,
        )
        self.enhance2 = NavieComplexLSTM(
            input_size=self.rnn_units,
            hidden_size=self.rnn_units,
            bidirectional=False,
            batch_first=False,
            projection_dim=4 * self.kernel_num[-1],
        )
        # if self.use_clstm:
        #     rnns = []
    ...
    def forward(self, inputs, state=None):
        ...
        if self.use_clstm:
            ...
            # [r_rnn_in, i_rnn_in], state_out = self.enhance([r_rnn_in, i_rnn_in], state)
            self.enhance = self.enhance1
            [r_rnn_in, i_rnn_in], state_out = self.enhance([r_rnn_in, i_rnn_in], state)
            self.enhance = self.enhance2
            [r_rnn_in, i_rnn_in], state_o = self.enhance2([r_rnn_in, i_rnn_in], state_out)
        ...
        for idx in range(len(self.decoder)):
            sk = encoder_out[-1-idx]
            out = complex_cat([out, sk], 1)
        ...
        return out_spec,  out_wav, state_o
...
if __name__ == '__main__':
    ...
    with torch.cuda.device(0):
        net = DCCRN(rnn_units=256,masking_mode='E',use_clstm=True,kernel_num=[32, 64, 128, 256, 256, 256])
        macs, params = get_model_complexity_info(net, (1, 16000))
