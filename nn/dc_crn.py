...
class DCCRN_mimo(nn.Module):
    ...
    def forward(self, inputs, label):
        ...
        inputs = inputs[0].reshape(-1, inputs[0].shape[-1])   # (B*C, T)
        ...
        cspecs = torch.concat([real, imag], dim=1)
        ...
        targets = label[0].reshape(-1, label[0].shape[-1])
        specs_t = self.stft(targets).reshape(batch, self.channel, self.fft_len + 2, -1)
        return est_phase,  out_wav.reshape(batch, self.channel, -1), specs_t
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        macs, params = profile(model, inputs=([wav.to(device)]))
