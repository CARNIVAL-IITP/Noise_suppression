...
class PMSQE(nn.Module):
    ...
    def __init__(self, window_name = 'sqrt_hann', window_weight = 1.0, bark_eq = True, gain_eq = True):
        ...
        self.abs_thresh_power = nn.Parameter(Tensor([
            ...
            0.524807,     0.524807,     0.524807]), requires_grad = False)
    ...
    0.530308,     0.536934]), requires_grad = False)

        self.sqrt_total_width = torch.sqrt(torch.sum(self.width_of_band_bark))

        self.bark_matrix = nn.Parameter(Tensor(loadmat('./losses/bark_matrix_8k.mat')["Bark_matrix_8k"]),
                                               requires_grad=False)
        mask_sll = np.zeros(shape=[np.int(self.nbins / 2 + 1)], dtype=np.float32)
